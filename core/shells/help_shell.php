...
/**
 * Help system for the WPKVC Console. 
 * Also provides the capability for shells to be self documenting.
 */
class HelpShell extends KvcShell {
    ...
    protected function get_shell_meta($name) {
        ...
        $name = KvcInflector::underscore($name);

        $shell = new stdClass();
        $shell->name = $name;
        $shell->shell_name = $name.'_shell';
        $shell->class_name = KvcInflector::camelize($shell->shell_name);
        $shell->title = KvcInflector::titleize($shell->name);
        $shell->methods = array();
    ...
    protected function get_available_shells() {
        $exclude = array(
            'kvc_shell.php',
            'kvc_shell_dispatcher.php'
        );

        $pluginAppPaths = KvcConfiguration::get('PluginAppPaths');
        ...
        foreach ($pluginAppPaths as $plugin => $path) {
            ...
            $key = KvcInflector::camelize($plugin);
     ...
     * Also is executed if the wpkvc console is run with no arguments.
     * 
     * @param mixed $args 
     */
    public function main($args) {
        ...
        foreach ($shells as $plugin => $shells) {
            ...
            $plugin_label = KvcInflector::camelize(KvcInflector::underscore($plugin));

            for ($i = 0; $i < count($shells); $i++) {
                if ($i > 0) {
                    $plugin_label = ' ';
                }
                $shell_name = KvcInflector::camelize($shells[$i]);
                $table->addRow(array(
                    $plugin_label,
        ...
        $this->out("\n\twpkvc help shell <name_of_shell>");
    }
     ...
     * Usage:
     * wpkvc Help Shell <shell_name> [command_name]
     * wpkvc Help Shell Generate
     * wpkvc Help Shell Generate Scaffold
     * @param mixed $args
     * @return null 
     */
