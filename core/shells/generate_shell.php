 ...
 */
class GenerateShell extends KvcShell {

    private $templater = null;

    public function init($args = array()) {
        $this->templater = new KvcTemplater();
        $this->templater->set_template_directory($this->core_path.'templates/plugin/app/');
    }
    
    /**
     * Generate a MVC Plugin.
     * wpkvc generate plugin <plugin>
     * @param mixed $args 
     */
    public function plugin($args) {
        if (empty($args[0])) {
            KvcError::fatal('Please specify a name for the plugin (e.g. "wpkvc generate plugin MyPlugin").');
        }
        $plugin = $args[0];
     ...
     * wpkvc generate controllers <plugin> <name>
     * @param mixed $args 
     */
     ...
     * wpkvc generate model <plugin> <model>
     * @param mixed $args 
     */
     ...
     * wpkvc generate scaffold <plugin> <model>
     * @param mixed $args 
     */
     ...
     * wpkvc generate views <plugin> <model>
     * @param mixed $args 
     */
     ...
     * wpkvc generate widget <plugin> <name>
     * @param array $args 
     */
    ...
    private function generate_app($plugin) {
        ...
        $plugin_camelized = KvcInflector::camelize($plugin);
        $plugin_titleized = KvcInflector::titleize($plugin);
        $plugin_underscored = KvcInflector::underscore($plugin);
    ...
    private function generate_controllers($plugin, $name) {
        ...
        $name_tableized = KvcInflector::tableize($name);
        $name_pluralized = KvcInflector::pluralize($name);
    ...
    private function generate_widget($plugin, $name) {
        ...
        $title = KvcInflector::titleize($name);
        
        $file_name = KvcInflector::underscore($name);
        
        $class_name = KvcInflector::camelize($plugin).'_'.KvcInflector::camelize($name);
        
        $name_underscore = KvcInflector::underscore($class_name);
    ...
    private function generate_model($plugin, $name) {
        $plugin_app_path = $this->get_plugin_app_path($plugin);
        $name_camelized = KvcInflector::camelize($name);
        $name_underscored = KvcInflector::underscore($name);
    ...
    private function generate_views($plugin, $name) {
        ...
        $name_tableized = KvcInflector::tableize($name);
        $name_titleized = KvcInflector::titleize($name);
        $name_titleized_pluralized = KvcInflector::pluralize($name_titleized);
        $name_underscored = KvcInflector::underscore($name);
    ...
    private function get_plugin_model_args($args) {
        if (empty($args[0]) || empty($args[1])) {
            KvcError::fatal('Please specify a plugin and name for the model.');
        }
        $args[0] = str_replace('_', '-', KvcInflector::underscore($args[0]));
        return $args;
    }
    
    private function get_plugin_path($plugin_name) {
        $plugin_underscored = KvcInflector::underscore($plugin_name);
        $plugin_hyphenized = str_replace('_', '-', $plugin_underscored);
