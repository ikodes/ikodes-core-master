...
class KvcRouter {
    ...
    static function public_url($options=array()) {
        $options = apply_filters('kvc_before_public_url', $options);
        $defaults = array(
            'action' => 'index',
            'controller' => null
        );
        ...
        if (!empty($options['object']) && is_object($options['object'])) {
            if (!empty($options['object']->__model_name) && !$controller) {
                ...
                $model_name = KvcInflector::camelize(KvcInflector::singularize($controller));
            } else {
                KvcError::warning('Incomplete arguments for KvcRouter::public_url().');
            }
    ...
    static function admin_url($options=array()) {
        if (!empty($options['object']) && is_object($options['object'])) {
            ...
            if (empty($options['controller']) && !empty($options['object']->__model_name)) {
                $options['controller'] = KvcInflector::tableize($options['object']->__model_name);
            }
        }
    ...
    static function admin_page_param($options=array()) {
        ...
        if (!empty($options['model'])) {
            return 'kvc_'.KvcInflector::tableize($options['model']);
        }
        return false;
    }

    static function public_connect( $route, $defaults = array() ) {
        $_this =& KvcRouter::get_instance();
    ...
    static function &get_instance() {
        ...
        if (!$instance) {
            $kvc_router = new KvcRouter();
            $instance[0] =& $kvc_router;
    ...
    static function maybe_add_route_prefix(&$route){
        $route_prefix = KvcConfiguration::get( 'CustomRoutePrefix' );
    ...
    static function get_route_prefix(){
        return KvcConfiguration::get( 'CustomRoutePrefix' );
    }
