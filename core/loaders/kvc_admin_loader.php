...
require_once 'kvc_loader.php';

class KvcAdminLoader extends KvcLoader {
    ...
    public function dispatch() {
        ...
        // If the beginning of $plugin_page isn't 'kvc_', then this isn't a WP MVC-generated page
        if (substr($plugin_page, 0, 4) != 'kvc_') {
            return false;
        }
        ...
        // Remove 'kvc_' from the beginning of the controller value
        ...
        if (!empty($controller)) {
            ...
            $kvc_admin_init_args = array(
                'controller' => $controller,
                'action' => $action
            );
            do_action('kvc_admin_init', $kvc_admin_init_args);
        
            $title = KvcInflector::titleize($controller);
            if (!empty($action) && $action != 'index') {
                $title = KvcInflector::titleize($action).' &lsaquo; '.$title;
            }
            $title = apply_filters('kvc_admin_title', $title);
    ...
    public function add_menu_pages() {
        ...
        $menu_position = apply_filters('kvc_menu_position', $menu_position);
        
        $admin_pages = KvcConfiguration::get('AdminPages');
        ...
        foreach ($this->admin_controller_names as $controller_name) {
            ...
            if (!$hide_menu) {
                ...
                try {
                    $model_name = KvcInflector::singularize(KvcInflector::camelize($controller_name));
                    $model = kvc_model($model_name);
                ...
                $controller_titleized = __(KvcInflector::titleize($controller_name), $this->plugin_name);
        
                $admin_controller_name = 'admin_'.$controller_name;
            
                $top_level_handle = 'kvc_'.$controller_name;

                $method = $admin_controller_name . '_index';
                $this->dispatcher->{$method} = function () use ($admin_controller_name) {
                    KvcDispatcher::dispatch(array(
    ...
    protected function process_admin_pages($controller_name, $pages) {
        ...
        $titleized = KvcInflector::titleize($controller_name);
    
        $default_pages = array(
            'add' => array(
                'label' => __('Add New', 'wpkvc')
            ),
            'delete' => array(
                'label' => __('Delete', 'wpkvc'). ' '.$titleized,
                'in_menu' => false
            ),
            'edit' => array(
                'label' => __('Edit', 'wpkvc'). ' '.$titleized,
                'in_menu' => false
            )
        );
        ...
        foreach ($pages as $key => $value) {
            ...
            $defaults = array(
                'action' => $key,
                'in_menu' => true,
                'label' => KvcInflector::titleize($key),
                'capability' => $capability
            );
    ...
    public function init_settings() {
        ...
        if (!empty($this->settings_names) && empty($this->settings)) {
            foreach ($this->settings_names as $settings_name) {
                $instance = KvcSettingsRegistry::get_settings($settings_name);
    ...
    public function add_settings_pages() {
        $this->init_settings();
        foreach ($this->settings as $settings_name => $settings) {
            $title = KvcInflector::titleize($settings_name);
    ...
    public function add_admin_ajax_routes() {
        $routes = KvcRouter::get_admin_ajax_routes();
        if (!empty($routes)) {
