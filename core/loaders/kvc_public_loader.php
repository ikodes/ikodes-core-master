...
class KvcPublicLoader extends KvcLoader {
    ...
    public function add_rewrite_rules($rules = array()) {
        ...
        $rules = apply_filters('kvc_public_rewrite_rules', $rules);
        return $rules;
    }
    ...
    public function get_new_rules() {   
        ...
        $routes = KvcRouter::get_public_routes();
        
        // Use default routes if none have been defined
        if (empty($routes)) {
            KvcRouter::public_connect('{:controller}', array('action' => 'index'));
            KvcRouter::public_connect('{:controller}/{:id:[\d]+}', array('action' => 'show'));
            KvcRouter::public_connect('{:controller}/{:action}/{:id:[\d]+}');
            $routes = KvcRouter::get_public_routes();
        }
    ...
    protected function get_rewrite_rules($route_path, $route_defaults, $controller, $first_query_var_match_index=0) {
        ...
        foreach ($matches[1] as $query_var) {
            $query_var = 'kvc_'.$query_var;
            if ($query_var != 'kvc_controller') {
        ...
        $controller_rules[$rewrite_path] = 'index.php?kvc_controller='.$controller_value.$query_var_match_string;
    ...
    {
        $vars = array();
        $params = KvcConfiguration::get('RouteParams');
        if ( isset( $params ) ) {
    ...
    public function template_redirect() {
        global $wp_query, $kvc_params;
        
        $routing_params = $this->get_routing_params();
        
        if ($routing_params) {
            $kvc_params = $routing_params;
            do_action('kvc_public_init', $routing_params);
    ...
    protected function get_routing_params() {
        ...
        $controller = $wp_query->get('kvc_controller');

        if ($controller) {
            $query_params = $wp_query->query;
            $params = array();
            foreach ($query_params as $key => $value) {
                $key = preg_replace('/^(kvc_)/', '', $key);
                $params[$key] = $value;
            }
