...
class KvcPluginLoader {
    ...
    protected function get_plugin_name_from_file_path($file_path) {
        ...
        KvcError::fatal('A plugin name couldn\'t be derived from the file path "'.$file_path.'"');
    }
    ...
    public function init_settings_defaults($file_path) {
        ...
        if (!empty($settings_files)) {
            foreach ($settings_files as $file_path) {
                $settings_class = KvcInflector::class_name_from_filename(basename($file_path));
