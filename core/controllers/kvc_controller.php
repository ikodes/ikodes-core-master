...
class KvcController {
    ...
    public function init() {
        ...
        $models = KvcModelRegistry::get_models();
        foreach ($models as $model_name => $model) {
            $underscore = KvcInflector::underscore($model_name);
            $tableize = KvcInflector::tableize($model_name);
        ...
        if (empty($this->model)) {
            $helper_name = class_exists('AppHelper') ? 'AppHelper' : 'KvcHelper';
        } else {
    ...
    private function set_meta() {
        ...
        $this->name = KvcInflector::underscore($model);
        $this->views_path = '';
        ...
        $model = KvcInflector::singularize($model);
        $this->views_path .= KvcInflector::tableize($model).'/';
    ...
    protected function load_helper($helper_name) {
        ...
        $helper_underscore = KvcInflector::underscore($helper_name);
    ...
    public function set_object() {
        ...
        if (!empty($object)) {
            ...
            KvcObjectRegistry::add_object($this->model->name, $this->object);
            return true;
        }
        KvcError::warning('Object not found.');
        return false;
    }
    ...
    protected function include_view($path, $view_vars=array()) {
        ...
        if (!$filepath) {
            ...
            $filepath = $this->file_includer->find_theme_or_view_file($path);
            if (!$filepath) {
                KvcError::warning('View "'.$path.'" not found.');
            }
        }
        require $filepath;
    }
    ...
    protected function set_flash($type, $message) {
        $this->init_flash();
        $_SESSION['kvc_flash'][$type] = $message;
    }
    
    protected function unset_flash($type) {
        $this->init_flash();
        unset($_SESSION['kvc_flash'][$type]);
    }
    
    protected function get_flash($type) {
        $this->init_flash();
        $message = empty($_SESSION['kvc_flash'][$type]) ? null : $_SESSION['kvc_flash'][$type];
        return $message;
    }
    
    protected function get_all_flashes() {
        $this->init_flash();
        return $_SESSION['kvc_flash'];
    }
    ...
    private function init_flash() {
        if (!isset($_SESSION['kvc_flash'])) {
            $_SESSION['kvc_flash'] = array();
        }
    }
    ...
    public function redirect($location, $status=302) {
        ...
        // KvcDispatcher::dispatch() doesn't run until after the WP has already begun to print out HTML, unfortunately, so
