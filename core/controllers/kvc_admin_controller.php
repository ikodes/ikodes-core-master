...
class KvcAdminController extends KvcController {
    ...
    public function delete() {
        ...
        if (!empty($this->object)) {
            ...
            $this->flash('notice', __('Successfully deleted!', 'wpkvc'));
        } else {
            $this->flash('warning', 'A '.KvcInflector::humanize($this->model->name).' with ID "'.$this->params['id'].'" couldn\'t be found.');
        }
        $url = KvcRouter::admin_url(array('controller' => $this->name, 'action' => 'index'));
        $this->redirect($url);
    }
        ...
        if (!empty($this->params['data'][$this->model->name])) {
            ...
            if (empty($object['id'])) {
                if($this->model->create($this->params['data'])) {
                    ...
                    $url = KvcRouter::admin_url(array('controller' => $this->name, 'action' => 'edit', 'id' => $id));
                    $this->flash('notice', __('Successfully created!', 'wpkvc'));
                    $this->redirect($url);
                } else {
    ...
    public function set_pagination($collection) {
        $url_params = KvcRouter::admin_url_params(array('controller' => $this->name));
        $params = $this->params;
    ...
    protected function init_default_columns() {
        if (empty($this->default_columns)) {
            KvcError::fatal('No columns defined for this view.  Please define them in the controller, like this:
                <pre>
                    class '.KvcInflector::camelize($this->name).'Controller extends KvcAdminController {
        ...
        foreach ($this->default_columns as $key => $value) {
            if (is_array($value)) {
                if (!isset($value['label'])) {
                    $value['label'] = KvcInflector::titleize($key);
                }
                ...
                if ($value == 'id') {
                    ...
                    $value = array('label' => KvcInflector::titleize($value));
                }
            } else {
