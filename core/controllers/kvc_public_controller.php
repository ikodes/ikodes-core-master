...
class KvcPublicController extends KvcController {
    ...
    public function set_pagination($collection) {
        ...
        $url = KvcRouter::public_url(array('controller' => $this->name, 'action' => $this->action));
    ...
    public function set_wp_title($original_title) {
        $separator = ' | ';
        $controller_name = KvcInflector::titleize($this->name);
        $object_name = null;
        $object = null;
        if ($this->action) {
        ...
        $title_options = apply_filters('kvc_page_title', array(
