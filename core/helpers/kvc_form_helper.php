...
class KvcFormHelper extends KvcHelper {
    ...
    public function create($model_name, $options=array()) {
        ...
        $this->object = KvcObjectRegistry::get_object($model_name);
        $this->model = KvcModelRegistry::get_model($model_name);
        ...
        if ($options['public']) {
            $html = '<form enctype="'.$options['enctype'].'" action="'.KvcRouter::public_url($router_options).'" method="post">';
        } else {
            $html = '<form enctype="'.$options['enctype'].'" action="'.KvcRouter::admin_url($router_options).'" method="post">';
        }
    ...
    public function input($field_name, $options=array()) {
        if (!empty($this->schema[$field_name])) {
            ...
            $defaults = array(
                'type' => $type,
                'label' => __(KvcInflector::titleize($schema['field']), $this->plugin_name),
            ...
            KvcError::fatal('Field "'.$field_name.'" not found for use in a form input.');
            return '';
        }
    }
    ...
    public function editor($field_name, array $options = array()) {
        ...
        $defaults = array(
            'id' => $id,
            'name' => $this->input_name($field_name),
            'label' => __(KvcInflector::titleize($field_name), $this->plugin_name),
    ...
    function select_from_model($field_name, KvcModel $model, $find_options = array(), $select_options = array()) {
    ...
    public function belongs_to_dropdown($model_name, $select_options, $options=array()) {
        ...
        if (!empty($this->model->associations[$model_name])) {
            ...
            $foreign_key = KvcInflector::underscore($model_name).'_id';
        }
        ...
        $defaults = array(
            ...
            'label' => __(KvcInflector::titleize($model_name), $this->plugin_name),
            'value' => $value,
    ...
    public function has_many_dropdown($model_name, $select_options, $options=array(), $associated_objects=false) {
        $defaults = array(
            ...
            'label' => __(KvcInflector::pluralize(KvcInflector::titleize($model_name)), $this->plugin_name),
            'options' => $select_options
        );
        $options = array_merge($defaults, $options);
        ...
        if($associated_objects === false){
            $associated_objects = isset($this->object->{KvcInflector::tableize($model_name)}) ? $this->object->{KvcInflector::tableize($model_name)} : '';
        }
    ...
    private function input_id($field_name) {
        return $this->model_name.KvcInflector::camelize($field_name);
    }

    private function input_name($field_name) {
        return 'data['.$this->model_name.']['.KvcInflector::underscore($field_name).']';
    }
