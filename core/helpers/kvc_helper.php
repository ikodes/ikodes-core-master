...
class KvcHelper {
    ...
    public function admin_actions_cell($controller, $object, $options = array()) {
        ...
        if($options['actions']['edit']){
            $links[] = '<a href="'.KvcRouter::admin_url(array('object' => $object, 'action' => 'edit')).'" title="' . __('Edit', 'wpkvc') . ' ' .$encoded_object_name.'">' . __('Edit', 'wpkvc') .'</a>';
        }
        
        if($options['actions']['view']){
            $links[] = '<a href="'.KvcRouter::public_url(array('object' => $object)).'" title="' . __('View', 'wpkvc') . ' ' .$encoded_object_name.'">' . __('View', 'wpkvc') .'</a>';
        }
        
        if($options['actions']['delete']){
            $links[] = '<a href="'.KvcRouter::admin_url(array('object' => $object, 'action' => 'delete')).'" title="' . __('Delete', 'wpkvc') . ' ' .$encoded_object_name.'" onclick="return confirm(&#039;' . __('Are you sure you want to delete', 'wpkvc') . ' ' .$encoded_object_name.'?&#039;);">' . __('Delete', 'wpkvc') .'</a>';
        }
    ...
    // To do: move this into an KvcUtilities class (?)
