...
class KvcModel {
    ...
    function __construct() {
        ...
        $table = empty($this->table) ? $wpdb->prefix.KvcInflector::tableize($this->name) : self::process_table_name($this->table);
        ...
        $this->db_adapter = new KvcDatabaseAdapter();
        $this->db_adapter->set_defaults($defaults);
        
        $this->data_validator = new KvcDataValidator();
        
        $this->init_schema();
        
        if ($this->wp_post) {
            $this->wp_post_adapter = new KvcPostAdapter();
    ...
    public function new_object($data) {
        $object = new KvcModelObject($this);
            ...
            foreach ($this->associations as $association) {
                if ($association['dependent']) {
                    if ($association['type'] == 'has_many') {
                        $model = KvcModelRegistry::get_model($association['class']);
                        foreach ($objects as $object) {
                            $options = array(
    ...
    protected function invalidate( $data, $field, $message ){
        if($this->validation_mode == 'all'){
            $this->validation_error[] = new KvcDataValidationError($field, $message);
            $this->validation_error_html = '';
        }else{
            $this->validation_error = new KvcDataValidationError($field, $message);
            $this->validation_error_html = $this->validation_error->get_html();
    ...
    protected function process_objects($objects, $options=array()) {
        ...
        foreach ($objects as $key => $object) {
            ...
            if (get_class($object) != 'KvcModelObject') {
                ...
                foreach ($includes as $include_key => $include) {
                    ...
                    switch ($association['type']) {
                        case 'belongs_to':
                            ...
                            $object->{KvcInflector::underscore($model_name)} = $associated_object;
                            break;
                        ...
                        case 'has_many':
                            ...
                            $object->{KvcInflector::tableize($model_name)} = $associated_objects;
                            break;
            ...
            foreach ($this->belongs_to as $key => $value) {
                ...
                if (is_string($value)) {
                    ...
                    $config = array(
                        ...
                        'foreign_key' => KvcInflector::underscore($association_name).'_id',
                        'includes' => null,
                        'dependent' => false
                    );
                    ...
                    $config = array(
                        ...
                        'foreign_key' => empty($value['foreign_key']) ? KvcInflector::underscore($association_name).'_id' : $value['foreign_key'],
            ...
            foreach ($this->has_many as $key => $value) {
                ...
                if (is_string($value)) {
                    ...
                    $config = array(
                        ...
                        'name' => $association_name,
                        'class' => $association_name,
                        'foreign_key' => KvcInflector::underscore($this->name).'_id',
                        'fields' => null,
                        'includes' => null,
                        'dependent' => false
                    );
                    ...
                    $config = array(
                        ...
                        'name' => empty($value['name']) ? $association_name : $value['name'],
                        'class' => empty($value['class']) ? $association_name : $value['class'],
                        'foreign_key' => empty($value['foreign_key']) ? KvcInflector::underscore($this->name).'_id' : $value['foreign_key'],
            ...
            foreach ($this->has_and_belongs_to_many as $key => $value) {
                if (is_string($key) && is_array($value)) {
                    ...
                    $config = array(
                        ...
                        'name' => $association_name,
                        'class' => $association_name,
                        'foreign_key' => isset($value['foreign_key']) ? $value['foreign_key'] : KvcInflector::underscore($this->name).'_id',
                        'association_foreign_key' => isset($value['association_foreign_key']) ? $value['association_foreign_key'] : KvcInflector::underscore($association_name).'_id',
    ...
    protected function init_properties() {
        ...
        foreach ($this->associations as $association_name => $association) {
            ...
            if ($association['type'] == 'belongs_to') {
                $property_name = KvcInflector::underscore($association_name);
            } else if (in_array($association['type'], array('has_many', 'has_and_belongs_to_many'))) {
                $property_name = KvcInflector::tableize($association_name);
            }
            if ($property_name) {
    ...
    protected function check_for_obsolete_functionality() {
        $obsolete_attributes = array(
            'admin_pages' => array('should be defined as \'AdminPages\' in KvcConfiguration', 'http://wpkvc.org/documentation/1.2/66/adminpages/'),
            'admin_columns' => array('should be defined as \'default_columns\' in the admin controller', 'http://wpkvc.org/documentation/1.2/16/default_columns/'),
            'admin_searchable_fields' => array('should be defined as \'default_searchable_fields\' in the admin controller', 'http://wpkvc.org/documentation/1.2/18/default_searchable_fields/'),
            'admin_search_joins' => array('should be defined as \'default_searchable_joins\' in the admin controller', 'http://wpkvc.org/documentation/1.2/17/default_search_joins/'),
            'public_searchable_fields' => array('should be defined as \'default_searchable_fields\' in the public controller', 'http://wpkvc.org/documentation/1.2/18/default_searchable_fields/'),
            'public_search_joins' => array('should be defined as \'default_searchable_joins\' in the public controller', 'http://wpkvc.org/documentation/1.2/17/default_search_joins/'),
            'hide_menu' => array('should be defined as \'in_menu\' in \'AdminPages\' in KvcConfiguration', 'http://wpkvc.org/documentation/1.2/66/adminpages/')
        );
    ...
    public function __call($method, $args) {
        if (substr($method, 0, 8) == 'find_by_') {
            ...
            if (isset($this->schema[$attribute])) {
                ...
                KvcError::fatal('Undefined attribute: '.$attribute.' for class: '.get_class($this).' when calling: '.$method.'.');
            }
        }
        ...
        KvcError::fatal('Undefined method: '.get_class($this).'::'.$method.'.');
    }
