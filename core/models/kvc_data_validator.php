...
class KvcDataValidator {
    ...
    public function validate($field, $value, $rule) {
        if (is_string($rule)) {
            if (method_exists($this, $rule)) {
                ...
                $error = new KvcDataValidationError($field, $message);
                return $error;
            } else {
                KvcError::fatal(sprintf(__("The validation rule %s wasn't found.", 'wpkvc'), $rule));
            }
        }
        if (is_array($rule)) {
        ...
        KvcError::fatal(sprintf(__("The validation rule %s wasn't defined correctly.", 'wpkvc'), print_r($rule, true)));
    }
    ...
    private function process_message($message, $field) {
        $titleized_field = KvcInflector::titleize($field);
    ...
    private function alphanumeric($value) {
        ...
        if ($this->matches_pattern($value, $pattern)) {
            ...
            return __('{field} must only contain letters and numbers.', "wpkvc");
        }
    }
    ...
    private function numeric($value) {
        if (is_numeric($value)) {
            ...
            return __('{field} must be a number.', 'wpkvc');
        }
    }
    ...
    private function email($value) {
        ...
        if ($this->matches_pattern($value, $pattern)) {
            ...
            return __('{field} must be a valid email address.', 'wpkvc');
        }
    }
    ...
    private function not_empty($value) {
        ...
        if ($this->matches_pattern($value, $pattern)) {
            ...
            return __("{field} can't be empty.", 'wpkvc');
        }
    }
    ...
    private function url($value) {
        ...
        if ($this->matches_pattern($value, $pattern)) {
            ...
            return __('{field} must be a valid URL.', 'wpkvc');
        }
    }
