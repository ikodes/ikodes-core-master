...
class KvcModelObject {
    ...
    public function __get($property_name) {
        ...
        if ($this->__settings['model']['name'] == 'KvcPost') {
            if (!empty($this->post_type)) {
                if (substr($this->post_type, 0, 4) == 'kvc_') {
                    $model_name = KvcInflector::camelize(substr($this->post_type, 4));
        ...
        $class = empty($this->__model_name) ? 'KvcModelObject' : $this->__model_name;
        KvcError::warning('Undefined property: '.$class.'::'.$property_name.'.');
    }
        ...
        switch ($association['type']) {
            ...
            case 'has_and_belongs_to_many':
                ...
                $associated_objects = $model->find(array(
                    'selects' => $association['fields'],
                    'joins' => array(
                        'table' => KvcModel::process_table_name($association['join_table']),
