...
class KvcPostAdapter {

    public function verify_settings($model) {
        if (empty($model->schema)) {
            KvcError::warning('The schema for '.$model->name.' is empty. It\'s likely that the plugin generating this table isn\'t working, or you may need to deactivate and activate it.  Please make sure that the table "'.$model->table.'" exists.');
        } else if (!isset($model->schema['post_id'])) {
            KvcError::fatal('To associate posts with '.$model->name.', its table needs to have a column named "post_id" of type BIGINT(20).  Please run the following SQL to add and index the column:
<pre>
...
ALTER TABLE '.$model->table.' ADD INDEX (post_id);
    ...
    public function save_post($model, $object) {
        ...
        foreach ($fields as $key => $value) {
            if (substr($value, 0, 1) == '$') {
                ...
                if (!isset($object->$attribute)) {
                    KvcError::fatal('The attribute "'.$attribute.'" was not present when trying to save the post for a '.$model->name.' object.');
                }
                $post_data[$key] = $object->$attribute;
            } else if (substr($value, -2) == '()') {
                $method = substr($value, 0, strlen($value) - 2);
                if (!method_exists($model, $method)) {
                    KvcError::fatal('The method "'.$method.'" was not present when trying to save the post for a '.$model->name.' object.');
                }
        ...
        if ($post_id) {
            ...
            if (is_wp_error($post_id)) {
                KvcError::fatal('The post for '.$model->name.' could not be saved ('.$post_id->get_error_message().').');
            } else {
    ...
    public function register_post_type($model) {
        ...
        if (!post_type_exists($settings['post_type']['key'])) {
            $title = KvcInflector::titleize($model->name);
            $title_plural = KvcInflector::pluralize($title);
    ...
    public function get_post_type_key($model) {
        $key = empty($model->wp_post['post_type']['key']) ? KvcInflector::underscore($model->name) : $model->wp_post['post_type']['key'];
